...
void myData()
{
	float vertices[] = {
		...
		////Tresera
		// 0.5f,  0.5f, -1.0f,   7.0f, 1.0f,//4
		// 0.5f, -0.5f, -1.0f,   7.0f, 0.0f,//5
		//-0.5f, -0.5f, -1.0f,   0.0f, 0.0f,//6
		//-0.5f,  0.5f, -1.0f,   0.0f, 1.0f,//7
		////Der
		//0.5f,  0.5f, -1.0f,  1.0f, 1.0f, //8
		//0.5f, -0.5f, -1.0f,  1.0f, 0.0f, //9
		//0.5f, -0.5f, 0.0f,   0.0f, 0.0f, //10
		//0.5f,  0.5f, 0.0f,   0.0f, 1.0f, //11
		////Izq
		//-0.5f,  0.5f, 0.0f,   1.0f, 1.0f,//12
		//-0.5f, -0.5f, 0.0f,   1.0f, 0.0f,//13
		//-0.5f, -0.5f, -1.0f,  0.0f, 0.0f,//14
		//-0.5f,  0.5f, -1.0f,  0.0f, 1.0f,//15	
		////Sup
		//0.5f,  0.5f, -1.0f,  0.5f, 1.0f,//16
		//0.5f,  0.5f, 0.0f,   0.5f, 0.66f,//17
		//-0.5f,  0.5f, 0.0f,  0.25f, 0.66f,//18
		//-0.5f,  0.5f, -1.0f, 0.25f, 1.0f,//19
		////Inf
		// 0.5f, -0.5f, -1.0f,  0.5f, 0.33f,//20
		// 0.5f, -0.5f, 0.0f,   0.5f, 0.0f,//21
		//-0.5f, -0.5f, 0.0f,   0.25f, 0.0f,//22
		//-0.5f, -0.5f, -1.0f,  0.25f, 0.33f,//23
	};
...
void display(Shader shader)
{
	...
	//dist = glm::translate(view, glm::vec3(movX, movY, movZ));
	//dist = glm::rotate(view, glm::radians(rotX), glm::vec3(1.0, 0.0f, 0.0f));
	//dist = glm::rotate(view, glm::radians(rotY), glm::vec3(0.0, 1.0f, 0.0f));
	//dist = glm::rotate(view, glm::radians(rotZ), glm::vec3(0.0, 0.0f, 1.0f));
	...
	for (i=0; i < 5; i = i + 0.714285714) {
		model = glm::translate(glm::mat4(1.0f), glm::vec3(i, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.714285714f, 1.0f, 0.0f));
		shader.setMat4("model", model);
	...
	for (i = 0; i < 5; i = i + 0.714285714) {
		model = glm::translate(glm::mat4(1.0f), glm::vec3(i, 0.0f, 6.0f));
		model = glm::scale(model, glm::vec3(0.714285714f, 1.0f, 0.0f));
		shader.setMat4("model", model);
	...
	model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
	model = glm::scale(model, glm::vec3(0.1f, 0.8f, 0.0f));
	shader.setMat4("model", model);
	shader.setVec3("aColor", glm::vec3(1.0f, 1.0f, 1.0f));
